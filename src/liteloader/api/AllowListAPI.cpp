...
bool AllowListManager::has(const std::string& name, const std::string& xuid) {
    size_t _;
    return has(name, xuid, _);
}

bool AllowListManager::has(const std::string& name, const std::string& xuid, size_t& index) {
    for (size_t i = 0, mEnd = allowList.size(); i < mEnd; ++i) {
        ...
        if (p["name"] == name) {
            if (xuid.empty()) {
                index = i;
                return true;
            } else if (p.count("xuid") && p.at("xuid") == xuid) {
                index = i;
                return true;
            }
        }
...
AllowListManager& AllowListManager::add(const std::string& name, const std::string& xuid, bool ignore) {
    try {
        nlohmann::json j{
            {"name",               name  },
...
AllowListManager& AllowListManager::remove(const std::string& name, const std::string& xuid) {
    try {
        size_t i = -1;
        if (has(name, xuid, i)) {
            allowList.erase(i);
...
[[maybe_unused]] void AllowListManager::reload() {
